...
//                        img.setAttribute('onmousemove', 'jQuery(function($){ $("#graph_full").mousemove(function(e){console.log(e.pageX)})  })');
...
function sbox_init(id) {
	...
	addListener(document, 'mouseout', sboxGlobalMouseout);
...
var box = document.getElementById('graph_full_box_on');
var width = box.offsetWidth - 125;
jQuery(".imgtip").css({"display":"block","top":e.pageY + "px","left":Math.min((e.pageX + 5), width) + "px"});
                var stime = new CDate((timeControl.timeline.usertime() - timeControl.timeline.period()) * 1000).getZBXDate();

this.optimizeEvent(e);
//this.createBox();
                        var dim = getDimensions(this.dom_obj);

                        this.shifts.left = dim.left;
                        this.shifts.top = dim.top;
                ...
                userstarttime += Math.round(((this.mouse_event.left - dim.left) - (this.additionShiftL - this.shifts.left)) * this.px2time);
//console.log(new CDate(userstarttime * 1000).getZBXDate());
//jQuery(".imgtip").html(userstarttime);
jQuery(".imgtip").html(new CDate(userstarttime * 1000).format("yyyy-MM-dd"));


		if (this.is_active) {
                        jQuery('.imgtip').css("display","none");
			this.optimizeEvent(e);
...
function sboxGlobalMouseout(e) {
	for (var id in ZBX_SBOX) {
		if (!empty(ZBX_SBOX[id])) {
			ZBX_SBOX[id].mouseOut(e);
		}
	}
}
